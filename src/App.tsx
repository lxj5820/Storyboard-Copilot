...
} from './features/update/checkForUpdate';
import {
      ...
      for (const [providerId, apiKey] of Object.entries(apiKeys)) {
        if (apiKey) {
          try {
            ...
            console.log('[App] Loaded API key for provider:', providerId);
          } catch (err) {
            console.error('[App] Failed to set API key for provider:', providerId, err);
          }
        }
      }
    };
