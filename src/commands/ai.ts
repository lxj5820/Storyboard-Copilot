...
export async function setProviderEndpoint(provider: string, endpoint: string): Promise<void> {
  console.info('[AI] set_provider_endpoint', {
    provider,
    endpoint,
    tauri: isTauri(),
  });
  ...
  return await invoke('set_provider_endpoint', { provider, endpoint });
}
...
export async function submitGenerateImageJob(request: GenerateRequest): Promise<string> {
  ...
  try {
    ...
    if (typeof jobId !== 'string' || !jobId.trim()) {
      throw createErrorWithDetails('submit_generate_image_job returned invalid job id');
    }
    const elapsedMs = Math.round(performance.now() - startedAt);
    console.info('[AI] submit_generate_image_job success', { elapsedMs, jobId });
    return jobId.trim();
  } catch (error) {
    const elapsedMs = Math.round(performance.now() - startedAt);
    const normalizedError = normalizeInvokeError(error);
    console.error('[AI] submit_generate_image_job failed', {
      elapsedMs,
      request: sanitizeGenerateRequestForLog(request),
      error,
      normalizedError,
...
export async function getGenerateImageJob(jobId: string): Promise<GenerationJobStatus> {
  const startedAt = performance.now();
  console.info('[AI] get_generate_image_job request', { jobId, tauri: isTauri() });
  ...
  try {
    ...
    if (!result || typeof result !== 'object' || typeof result.status !== 'string') {
      throw createErrorWithDetails('get_generate_image_job returned invalid payload');
    }
    const elapsedMs = Math.round(performance.now() - startedAt);
    console.info('[AI] get_generate_image_job success', { elapsedMs, jobId, status: result.status });
    return result;
  } catch (error) {
    const elapsedMs = Math.round(performance.now() - startedAt);
    const normalizedError = normalizeInvokeError(error);
    console.error('[AI] get_generate_image_job failed', {
      elapsedMs,
      jobId,
      error,
      normalizedError,
...
export async function listModels(): Promise<string[]> {
  const startedAt = performance.now();
  console.info('[AI] list_models request', { tauri: isTauri() });
  ...
  try {
    ...
    if (!Array.isArray(result)) {
      throw createErrorWithDetails('list_models returned non-array payload');
    }
    const elapsedMs = Math.round(performance.now() - startedAt);
    console.info('[AI] list_models success', { elapsedMs, modelCount: result.length });
    return result;
  } catch (error) {
    const elapsedMs = Math.round(performance.now() - startedAt);
    const normalizedError = normalizeInvokeError(error);
    console.error('[AI] list_models failed', {
      elapsedMs,
      error,
      normalizedError,
    });
