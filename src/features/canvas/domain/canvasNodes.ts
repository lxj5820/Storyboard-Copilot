...
export interface TextInputNodeData extends NodeDisplayData {
  content: string;
  [key: string]: unknown;
}
...
export function isTextInputNode(
  node: CanvasNode | null | undefined
): node is Node<TextInputNodeData, typeof CANVAS_NODE_TYPES.textInput> {
  return node?.type === CANVAS_NODE_TYPES.textInput;
}
