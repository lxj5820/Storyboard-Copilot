...
const dataUrlCache = new Map<string, { dataUrl: string; timestamp: number }>();
const CACHE_EXPIRY_MS = 1000 * 60 * 5; // 5分钟缓存
...
export async function imageUrlToDataUrl(imageUrl: string): Promise<string> {
  ...
  if (isLikelyLocalImagePath(imageUrl)) {
    if (isTauri()) {
      try {
        dataUrl = await loadImage(imageUrl);
      } catch (error) {
      ...
      dataUrl = await blobToDataUrl(localBlob);
    }
  } else {
