...
import {
  canvasAiGateway,
  graphImageResolver,
} from '@/features/canvas/app/canvasServices';
import { resolveErrorContent, showErrorDialog } from '@/features/canvas/app/errorDialog';
import {
  detectAspectRatio,
  parseAspectRatio,
  resolveImageDisplayUrl,
} from '@/features/canvas/app/imageData';
import {
...
} from '@/features/canvas/app/generationErrorReport';
import {
  findReferenceTokens,
  insertReferenceToken,
  removeTextRange,
  resolveReferenceAwareDeleteRange,
} from '@/features/canvas/app/referenceTokenEditing';
import {
...
import { CUSTOM_NANO_BANANA_2_MODEL_ID } from '@/features/canvas/models/image/custom/nanoBanana2';
...
export const ImageEditNode = memo(({ id, data, selected, width, height }: ImageEditNodeProps) => {
  ...
  const modelPrices = useSettingsStore((state) => state.modelPrices);
  ...
  const incomingTexts = useMemo(
    () => graphImageResolver.collectInputTexts(id, nodes, edges),
    [id, nodes, edges]
  );
  ...
  const enabledModels = useSettingsStore((state) => state.enabledModels);
  const customModels = useSettingsStore((state) => state.customModels);
  const imageModels = useMemo(() => listImageModels(), [enabledModels, customModels]);
  ...
  // 获取 API Key：优先从 customModels 获取，否则从 apiKeys 获取
  const providerApiKey = useMemo(() => {
    // 尝试从 customModels 中获取当前模型的 API Key
    const customModel = customModels.find(m => m.id === selectedModel.id);
    if (customModel?.apiKey) {
      return customModel.apiKey;
    }
    // 回退到从 apiKeys 获取
    return apiKeys[selectedModel.providerId] ?? '';
  }, [selectedModel.id, selectedModel.providerId, apiKeys, customModels]);
  ...
  const currentEndpointType = useMemo(() => {
    return incomingImages.length > 0 ? '图生图' : '文生图';
  }, [incomingImages.length]);
  const showWebSearchToggle = selectedModel.id === CUSTOM_NANO_BANANA_2_MODEL_ID;
  ...
  const resolvedPriceDisplay = useMemo(
    () => {
      ...
      const customPrice = modelPrices[selectedModel.id];
      const language = i18n.language;
      const displayCurrencyMode = priceDisplayCurrencyMode;
      const rate = usdToCnyRate;
      
      if (customPrice && customPrice > 0) {
        const currency = displayCurrencyMode === 'cny' ? 'CNY' : (displayCurrencyMode === 'usd' ? 'USD' : (language.startsWith('zh') ? 'CNY' : 'USD'));
        const amount = currency === 'CNY' ? customPrice * rate : customPrice;
        const perRunSuffix = language.startsWith('zh') ? '/次' : '/run';
        const formatter = new Intl.NumberFormat(language.startsWith('zh') ? 'zh-CN' : 'en-US', {
          style: 'currency',
          currency,
        ...
        return {
          label: `${formatter.format(amount)}${perRunSuffix}`,
          customPrice: true,
          nativeLabel: undefined,
          originalLabel: undefined,
          pointsCost: undefined,
        grsaiCreditTier: undefined,
        };
      }
      
      return resolveModelPriceDisplay(selectedModel, {
  ...
  const handleGenerate = useCallback(async () => {
    // 当有文本输入时，完全替代内置提示词
    const textInputContent = incomingTexts.join(' ').trim();
    let prompt = '';
    
    if (textInputContent) {
      // 验证文本长度
      const maxPromptLength = 4000; // 根据AI模型的限制设置
      if (textInputContent.length > maxPromptLength) {
        const errorMessage = t('node.imageEdit.promptTooLong', { maxLength: maxPromptLength });
        setError(errorMessage);
        void showErrorDialog(errorMessage, t('common.error'));
        return;
      }
      
      // 验证文本格式（可以根据需要添加更多验证规则）
      if (!/^[\s\S]*$/.test(textInputContent)) {
        const errorMessage = t('node.imageEdit.invalidPromptFormat');
        setError(errorMessage);
        void showErrorDialog(errorMessage, t('common.error'));
        return;
      }
      ...
      // 没有文本输入时，使用内置提示词
      const promptContent = promptDraft.replace(/@(?=图\d+)/g, '').trim();
